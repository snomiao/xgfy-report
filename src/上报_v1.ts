...
async function 上报系统构建({ 启用bot } = { 启用bot: false }) {
    ...
    const 信息抓取更新 = async ({ _id }) => await API请求('/report/report/selectByCode', { code: _id })
        .then(返回JSON => {
            if (返回JSON?.code !== 0)
                throw new EvalError(`${_id} 信息返回代码异常：` + JSON.stringify(返回JSON));
    ...
    const 旧信息抓取更新通知 = async () => await pMap(
        await db.上报者信息.聚合([
            { $match: { 更新于: 较早于(-30 * 86400e3) } }, { $project: { _id: 1 } }, { $sample: { size: 100 } }
        ]).toArray(),
        信息抓取更新, { concurrency: 3, stopOnError: false })
        .then(async (表列) => 表列.length && await 通知('信息更新' + 表列.join(' ')));
    const 状态抓取 = async ({ _id }) => await API请求('/report/report/getMyReport', { usercode: _id })
        .then(返回 => {
            if (返回.code !== 0)
                throw new Error((`${_id}上报数据获取异常：${JSON.stringify(返回)}`));
    ...
    const 状态上报 = async ({ _id, ...状态 }) => {
        if (!状态)
            throw new Error((`${_id}昨日数据格式异常：${JSON.stringify(状态)}`));
    ...
    const 上报异常者状态抓取更新 = async () => await pMap(
        await db.上报者状态.聚合([
            { $match: { batchno: { $ne: 今日批号获取() }, 计划于: 较早于(0), 更新于: 较早于(-37 * 60e3 /* 约 37分钟访问一次  */ * 随机衰减()) } }, { $project: { _id: 1 } }, { $sample: { size: 100 } }
        ]).toArray(),
        状态抓取更新, { concurrency: 3, stopOnError: false }).then(e => (`✅上报异常者状态抓取更新_x` + e?.length)).catch(console.error);
    const 上报异常者上报 = async () => await pMap(
        await db.上报者状态.聚合([
            { $match: { batchno: { $ne: 今日批号获取() }, 计划于: 较早于(0) } }, { $sample: { size: 100 } }
        ]).toArray(),
    ...
    const 保护上报异常者上报 = async () => await pMap(
        ...
        状态上报更新, { concurrency: 3, stopOnError: false }).then(e => (`✅保护上报异常者上报_x` + e?.length)).catch(console.error);
    const 上报异常者通知 = async () => await db.上报者状态.聚合([
        { $match: { batchno: { $ne: 今日批号获取() }, 更新于: 较早于(-57 * 60e3 /* 57分钟前 */) } }, { $sample: { size: 100 } }
    ]).toArray()
    ...
    const 上报完成者通知 = async () => await db.上报者状态.聚合([
        { $match: { batchno: { $eq: 今日批号获取() }, 通知批号: { $ne: 今日批号获取() } } }, { $sample: { size: 100 } }
    ]).toArray()
        .then(async (表列) => (表列.length && await Promise.resolve(表列)
    ...
    const 自动上报异常警报 = async () => await db.上报者状态.聚合([
        { $match: { 自动: true, batchno: { $ne: 今日批号获取() }, /* 异常通知批号: { $ne: 今日批号获取() } */ } }, { $sample: { size: 100 } }
    ]).toArray()
        .then(async (表列) => (表列.length && await Promise.resolve(表列)
            .then(async () => await 警报('自动上报异常：' + 表列.map(e => 今日状态表述(e)).join('\n')))
            .then(async () => await db.上报者状态.多补(表列.map(({ _id }) => ({ _id, 异常通知批号: 今日批号获取() }))))
            .then(async () => `✅自动上报异常警报_x${表列.length}`)));
    const 保护上报异常通知 = async () => await db.上报者状态.聚合([
        { $match: { 保护: true, batchno: { $ne: 今日批号获取() }, /* 异常通知批号: { $ne: 今日批号获取() } */ } }, { $sample: { size: 100 } }
    ]).toArray()
        .then(async (表列) => (表列.length && await Promise.resolve(表列)
            .then(async () => await 通知('保护上报异常：' + 表列.map(e => 今日状态表述(e)).join('\n')))
            .then(async () => await db.上报者状态.多补(表列.map(({ _id }) => ({ _id, 异常通知批号: 今日批号获取() }))))
            .then(async () => `✅保护上报异常通知_x${表列.length}`)));
    const 无效上报者禁用通知 = async () => await db.上报者状态.聚合([
        { $match: { username: null } }, { $sample: { size: 100 } }
    ]).toArray()
        .then(async (表列) => (表列.length && await Promise.resolve(表列)
...
export async function 自动上报警报流程() {
    ...
    while (+new Date() - 启动时刻 < 3600e3) { //运行1小时后退出，由docker来自动重启
        ...
        if (现在HK小时数() >= 10) console.log('保护上报异常者上报', await 系统.保护上报异常者上报()) //10点后才用使用保护
        if (现在HK小时数() >= 11) console.log('保护上报异常通知', await 系统.保护上报异常通知()) //11点后才用使用通知
        console.log('无效自动上报者禁用警报', await 系统.无效自动上报者禁用警报())
        console.log('无效上报者禁用通知', await 系统.无效上报者禁用通知())
        if (现在HK小时数() >= 3) console.log('自动上报异常警报', await 系统.自动上报异常警报()) //3点后才用使用警报
        if (现在HK小时数() >= 11) console.log('上报异常者通知', await 系统.上报异常者通知()) //11点后才用使用通知
...
function 现在HK小时数() {
    return new Date(+new Date() + 8 * 3600e3).getUTCHours();
}
